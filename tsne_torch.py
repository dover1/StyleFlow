...
if __name__ == "__main__":
    ...
    parser.add_argument("--file_path", type=str,
                        default="/home/hyh/PycharmProjects/nf/save/yelp/separate=False_flowType=realNVP_flowNum=6_seed=11_nf=0/results",
                        help="file name of feature stored")
    ...
    iter = np.arange(0, 11, 1)
    types = ['c_x', 'z', 'trans_x']
    for i in iter:
        for type in types:
            X_pos, X_neg = 0, 0
            x_pos_file = opt.file_path + f"/epoch_{i}_{type}.1"
            x_neg_file = opt.file_path + f"/epoch_{i}_{type}.0"
            #
            # X_pos = np.load(x_pos_file)
            # X_neg = np.load(x_neg_file)
            # dist = np.matmul(np.mean(X_pos, axis=0), np.mean(X_neg, axis=0))
            # print(dist)
            X_pos = nn.functional.normalize(torch.load(x_pos_file), p=2, dim=-1).cpu()
            X_neg = nn.functional.normalize(torch.load(x_neg_file), p=2, dim=-1).cpu()
            ...
            plt.savefig(opt.file_path + f"/epoch_{i}_{type}_tsne.png")
            plt.show()
