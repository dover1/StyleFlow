...
def train_lm(config, log, vocab, model, train_loader):
    ...
    out_dir = config.pretrain_lm_ckpt_dir
    ...
    for epoch in range(config.pretrain_epochs):
        for step, batch in enumerate(train_loader):
            ...
            f_loss = seq_crit(f_logits, inputs[:, 1:], truncate=True)
            b_loss = seq_crit(b_logits, inputs[:, 1:], truncate=True)
...
def pretrain(config, log, vocab, model_G, train_loader, load_ckpt=True):
    ...
    if load_ckpt:
        try:
            pretrain_file = os.path.join(config.pretrain_lm_ckpt_dir, "epoch{}.pt".format(config.pretrain_epochs))
    ...
    train_lm(config, log, vocab, model_G, train_loader)
