...
class Encoder(nn.Module):
    def __init__(self, config):
        ...
        # self.enc2dec_h = nn.Sequential(nn.Linear(self.hidden_size * self.hidden_factor, config.hidden_size), nn.Tanh())
    ...
    def forward(self, inputs, lengths=None):
        ...
        if lengths is None:
            outputs, state_h = self.rnn(input_embeds, None)
        else:
            # Dynamic RNN
            packed = torch.nn.utils.rnn.pack_padded_sequence(input_embeds, lengths.cpu(), batch_first=True, enforce_sorted=False)
            outputs, state_h = self.rnn(packed, None)
            # outputs: (B, L, 2*H)
        ...
        #     state_h = state_h.transpose(0, 1).contiguous().view(batch_size, self.hidden_size*self.hidden_factor)
        # else:
        #     state_h = state_h.squeeze()
        #
        # hidden = self.enc2dec_h(state_h)
...
class Decoder(nn.Module):
    def __init__(self, config):
        ...
        self.linear = nn.Sequential(
            nn.Linear(config.embedding_size + config.hidden_size*bidirectional, config.embedding_size),
            nn.Tanh()
        )

    def forward(self, input_embeds, last_state, enc_outs, attention_mask):
        # input_embeds: (B, 1, emb_size)
        embedded = self.dropout(input_embeds)

        if enc_outs is not None:
            # use h_t as query
            query = last_state.transpose(0, 1)  # (B, 1, H)
